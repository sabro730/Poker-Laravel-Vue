...
class UserController extends Controller
{
    ...
    {
        $pageTitle = 'Dashboard';
        $games = Game::where('status',1)->get(['image','name','id','max_limit','min_limit','win','invest_back','alias']);
        return view($this->activeTemplate . 'user.dashboard', compact('pageTitle','games'));
    }
    ...
    {
        $pageTitle = "Game Logs";
        $logs = GameLog::where('user_id', auth()->id())->where('status',1)->latest('id')->with('game')->paginate(getPaginate());
        return view($this->activeTemplate . 'user.gameLog', compact('pageTitle', 'logs'));
    }

    public function commissionLog()
    {
        $pageTitle = "Commission Logs";
        $logs = CommissionLog::where('user_id', \auth()->id())->latest('id')->paginate(getPaginate());
        return view($this->activeTemplate . 'user.commissionLog', compact('pageTitle', 'logs'));
    }
    ...
    public function transactions()
    {
        ...
        $logs = Transaction::where('user_id', \auth()->id())->latest('id')->paginate(getPaginate());
        return view($this->activeTemplate . 'user.transactions', compact('pageTitle', 'logs'));
    }
